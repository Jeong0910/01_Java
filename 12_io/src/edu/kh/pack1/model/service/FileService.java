...
public class FileService {
	...
	/**
	 * File 클래스로 객체 생성 + 폴더가 존재하지 않으면 원하는 경로에 폴더를 생성
	 */
	public void method1() {
		...
		// io_test/2025101 앞에 슬래시 없이 작성 시 해당 프로젝트가 현재 위치!!
		// 해당 프로젝트 안에 io_test/20251014
		// -> 현재 프로젝트 : 12_io
		// ㄴio_test
		// ㄴ20251014
	...
	public void method2() {
		...
		if (!file.exists()) { // 존재하지 않으면
			...
			try {
				if(file.createNewFile()) { // 파일이 정상 경우
	...
	public void method3() {
		...
		// File 객체 생성
		File directory = new File("\\worksapce\\01_Java\\12_io");
		...
		for(File f : files) {
			...
			// 마지막으로 수정한 날짜
			Long lastmodifed = f.lastModified(); // 알수없는 엄청 긴 숫자가 뜸
