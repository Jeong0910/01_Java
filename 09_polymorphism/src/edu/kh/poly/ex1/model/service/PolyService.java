...
// 자식타입 참조변수 = 자식객체주소
		...
		// Telsa 부모, 본인의 메서드 모두 사용 가능 (일반적인 상속관계)
		
		// 1-3) car2 (부모 = 자식(Telsa))
		car2.setEngine("전기모터");
		car2.setFuel("전기");
		car2.setWheel(4);
		// car2.setBatteryCapacity(1000000); -> 본인꺼는 사용할 수가 없다
		...
		arr[0] = car; // Car 주소값
		arr[1] = car2; // Tesla 주소값
		// Car 타입 참조변수 = 자식(Tesla) 객체주소
		arr[2] = car3; // Spark 주소값
		// Car 타입 참조변수 = 자식(Spark) 객체주소
		
		for(int i = 0; i < arr.length; i++) {
			System.out.println(i + "번째 인덱스 엔진 : " + arr[i].getEngine());
		}
	...
	// 전달받은 Car 또는 자식객체(Tesla, Spark)의
	// 엔진, 연료, 바퀴갯수를 출력하는 메서드
	...
	public void ex2() {
		...
		// 다형성을 이용한 반환형 사용법
		// Cark[] arr = { new Car(), new Tesla(), new Spark() }
		
		Car[] arr = {createCar(1),createCar(2),createCar(3)}; // 반환하는 값을 요소로 쓰고싶다?!
		
		// 객체가 어떤 타입(자료형)인지 검사
		// instanceof 연산자 : 객체의 자료형을 검사하는 연산자
		// -> 참조하는 객체가 특정 자료형이거나 상속관계인지 확인
		
		System.out.println(arr[1] instanceof Tesla); // true (자기 본인 확인)
		System.out.println(arr[1] instanceof Spark); // false
		// Tesla와 Spark는 같은 부모 Car을 갖고 있을 뿐 아무런 관계 아님
		System.out.println(arr[1] instanceof Car); // true (상속관계)
	}
	...
	public void ex4() {
		...
		if(c1 instanceof Spark) { // c1이 Spark 이거나 상속관계인가?
			...
			System.out.println("Fail. c1 is not relation to type / inheritance with Spark");
		}
	}
	...
	public void ex5() {
		...
		// -> "실행 시"에는 Spark에 오러라이딩된 toString()이
