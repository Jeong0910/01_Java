...
public class MemberService {
	...
	public MemberService() {// 기본생성자
		...
		memberArr[0] = new Member("user01","pass01","엘프",279,"서울");
		memberArr[1] = new Member("user02","pass02","인간",25,"경기");
		memberArr[2] = new Member("user03","pass03","드워프",46,"강원");
	...
	public void displayMenu() {
		...
		do {
			...
			default : System.out.println("잘못 입력하셨습니다." + "메뉴에 잇는 번호만 선택하세요!");
			}
		} while(menuNum != 0); //munuNum이 0이 되면 반복 종료
		
	}
	// 사이드 메서드 : 메인 기능은 아니나, 다른 메서드에 도움이 되는 메서드
	// memberArr에 비어있는 인덱스 번호를 반환하는 역할
	...
	public String selectMember() {
		...
		// 1) 로그인 여부 확인
		// 필드 loginMember 가 참조하고있는 객체 있는지 확인
		if(loginMember == null) {
	...
	public int updateMember() {
		...
		if(pw01.equals(loginMember.getMemberPw() ) ) { // 같은 경우
			loginMember.setMemberName(name01);
			loginMember.setMemberAge(age01);
			loginMember.setRegion(region01);
			loginMember.setMemberPw(pw01);
